...
for i = 2:nsamples
    ...
    index = 1:len;
    sorted_dist = [index; distances];
    for v = 1:len
        for w = (len):-1:(v+1)
            if(sorted_dist(2, w) < sorted_dist(2, w-1))
                tmp = sorted_dist(1:end, w);
                sorted_dist(1:end, w) = sorted_dist(1:end, w-1);
                sorted_dist(1:end, w-1) = tmp;
            end
        end
    end
    ...
    for j = 1:k_closest
        collision_free = LocalPlanner(x(:), samples(:,sorted_dist(1, j)));
        if collision_free
            nedges = nedges + 1;
            edges(nedges,:) = [i, sorted_dist(1, j)];
            edge_lengths(nedges,:) = sorted_dist(2, j);
        end
    end
